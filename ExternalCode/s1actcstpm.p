/* s1actcstpm.p - Actualizar el costo REAL de material en base al costo     */
...
/* REVISION: 9.1     LAST MODIFIED: sep/11/20 BY: *DF00* Diego Fernandez    */
/****************************************************************************/
/* REV: EE   DATE: 18-NOV-2020  BY: *DF01* Diego Fernandez - Migracion a EE */
...
/*CM04*//* venta de la filial con true up más los gastos en uy.             */                     
         ...
         define variable iPeriodo       as integer format "99" 
            view-as radio-set radio-buttons s1actcstpm_lbl_opc1, 1,       /* Mes Ingresado */ 
                                            s1actcstpm_lbl_opc2, 2,       /* 1er Trimestre */ 
                                            s1actcstpm_lbl_opc3, 3,       /* 1er Semestre  */ 
                                            s1actcstpm_lbl_opc4, 4,       /* 1ros 9 Meses  */ 
                                            s1actcstpm_lbl_opc5, 5.       /* Anual         */ 
         ...
         repeat:
            ...
            do on error undo, retry:
               ...
               update
                  part 
                  part1 
               /* cGrupo *** DF01: no tiene sentido pedirlo porque en el CASE solo entrara si es "REAL" */ 
                  iPeriodo 
                  cTipoCosto
               with frame a.
               ...
               case iPeriodo:
                  when 1 then     /* periodo actual */
                     assign 
                        iPerDesde = per
                        iPerHasta = per.
                  when 2 then     /* periodo trimestre */
                     assign 
                        iPerDesde = 1
                        iPerHasta = 3.
                  when 3 then     /* periodo semestre */
                     assign 
                        iPerDesde = 1
                        iPerHasta = 6.
                  when 4 then     /* periodo 9 meses */
                     assign 
                        iPerDesde = 1
                        iPerHasta = 9.
                  when 5 then     /* periodo anual */
                     assign 
                        iPerDesde = 1
                        iPerHasta = 12.
               end.
               ...
               if cGrupo = "" then do:
                  ...
                  find first code_mstr no-lock
                     where code_domain  = global_domain
                     and   code_fldname = "tipo_proy"
                     and   code_value   = cGrupo no-error.
                  if not available code_mstr then do:
                     {us/bbi/pxmsg.i &MSGNUM=44209 &ERRORLEVEL=3} /* Grupo no parametrizado en codigos generalizados */
                     next-prompt cGrupo with frame a.
                     undo, retry.
                  end.
               end.
               ...
               if not available sc_mstr then do:
                  ...
                  next-prompt cTipoCosto with frame a.
                  undo, retry.
               end.
                     ...
                     for each s1_cstpc_det no-lock
                        ...
                        if cSubRegionFilial = cSubReg then do:
                           ...
                           find last s1_ptvp no-lock
                              ...
                              and   s1_ptvp_grupo  = "R" no-error.
                           if not available s1_ptvp then do:
                              find last s1_ptvp no-lock
                                 where s1_ptvp_domain = global_domain
                                 and   s1_ptvp_part   = s1_cstpc_producto
                                 and   s1_ptvp_filial = filial
