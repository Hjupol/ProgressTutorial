            ...
            do on error undo, retry:
               ...
               if lastkey = keycode("F5") or lastkey = keycode("CTRL-D") then do:                                                           
                  ...
                  for each ttFiliales where ttFiliales_domain <> global_domain:
                     find first s1_ptsig_mstr exclusive-lock
                        where s1_ptsig_domain = ttFiliales_domain
                        and   s1_ptsig_part   = s1ptsigpart no-error.
               ...
               find first s1_drogval_det exclusive-lock
                  where s1_drogval_domain = global_domain
                  and   s1_drogval_droga  = s1_ptsig_droga
                  and   s1_drogval_forma  = s1_ptsig_forma
                  and   s1_drogval_envase = s1_ptsig_envase 
                  no-error. 
               if not available s1_drogval_det then do:
                  yn = true.
                  /* 9931 DCF no existe en punto 80.1.31, desea continuar? */
                  {us/bbi/pxmsg.i &MSGNUM=9931 &ERRORLEVEL=1 &CONFIRM=yn}
                  if yn = no then do:
                     undo, retry.
                  end.
               end.
      ...
      procedure replicarRegistros:
         ...
         for each ttFiliales where ttFiliales_domain <> global_domain:
            ...
            find first bto exclusive-lock
               where bto.s1_ptsig_domain = ttFiliales_domain
               and   bto.s1_ptsig_part   = bfrom.s1_ptsig_part no-error.
            if not available bto then do:
               create bto.
               assign
                  bto.s1_ptsig_domain = ttFiliales_domain
                  bto.s1_ptsig_part   = bfrom.s1_ptsig_part.
            end.
            
            buffer-copy bfrom except s1_ptsig_domain s1_ptsig_part to bto.
